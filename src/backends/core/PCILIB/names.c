/*
 *	$Id: names.c,v 1.1 2001/10/06 09:23:21 davej Exp $
 *
...
static void err_name_list (struct pci_access *a, char *msg)
{
...
static void parse_name_list (struct pci_access *a)
{
	...
	while (*p) {
		...
		if (q == r) {
			if (q[0] == 'C' && q[1] == ' ') {
				if (strlen (q + 2) < 3 || q[4] != ' ' || sscanf (q + 2, "%x", &id1) != 1)
					goto parserr;
				cat = NL_CLASS;
			} else {
...
void recursive_free_hash(struct nl_entry *n)
{
	if (n==NULL)
		return;
	recursive_free_hash(n->next);
	pci_mfree(n);
}
...
void pci_free_name_list (struct pci_access *a)
{
	...
	if (a->nl_hash!=NULL)
		for (i=0;i<HASH_SIZE;i++) {
			recursive_free_hash(a->nl_hash[i]);
			a->nl_hash[i] = NULL;
		}
	pci_mfree(a->nl_hash);
	a->nl_hash = NULL;
