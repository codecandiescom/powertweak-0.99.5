/*
 *	$Id: proc.c,v 1.1 2001/10/06 09:23:21 davej Exp $
 *
...
#if defined(__GLIBC__) && __GLIBC__ == 2 && __GLIBC_MINOR__ > 0  && defined(HAVE_PREAD)
/* glibc 2.1 or newer -> pread/pwrite supported automatically */

#elif defined(i386) && defined(__GLIBC__) && defined(HAVE_PREAD)
...
#elif defined(i386) && defined(HAVE_PREAD)
...
static int do_read (struct pci_dev *d UNUSED, int fd, void *buf, size_t size, int where)
{
	return pread (fd, buf, size, where, 0);
}
static int do_write (struct pci_dev *d UNUSED, int fd, void *buf, size_t size, int where)
{
	return pwrite (fd, buf, size, where, 0);
}
...
{
	a->method_params[1] = PATH_PROC_BUS_PCI;
}

static int proc_detect (struct pci_access *a)
{
	char *name = a->method_params[1];
...
static void proc_scan (struct pci_access *a)
{
	...
	if (snprintf (buf, sizeof (buf), "%s/devices", a->method_params[1]) == sizeof (buf))
	...
	while (fgets (buf, sizeof (buf) - 1, f)) {
		...
		cnt = sscanf (buf,
			      ...
			      &d->rom_base_addr,
			      &d->size[0],
			      &d->size[1], &d->size[2], &d->size[3], &d->size[4], &d->size[5], &d->rom_size);
...
static int proc_setup (struct pci_dev *d, int rw)
{
	...
	if (a->cached_dev != d || a->fd_rw < rw) {
		...
		if (a->fd >= 0)
			close (a->fd);
		if (snprintf (buf, sizeof (buf), "%s/%02x/%02x.%d", a->method_params[1],
