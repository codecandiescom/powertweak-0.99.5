/*
 *	$Id: generic.c,v 1.1 2001/10/06 09:23:21 davej Exp $
 *
...
int pci_generic_fill_info (struct pci_dev *d, int flags)
{
			...
			for (i = 0; i < cnt; i++) {
				...
				if (x & PCI_BASE_ADDRESS_SPACE_IO) {
					...
					if ((x & PCI_BASE_ADDRESS_MEM_TYPE_MASK) == PCI_BASE_ADDRESS_MEM_TYPE_64) {
						if (i >= cnt - 1)
							a->warning ("%02x:%02x.%d: Invalid 64-bit address seen.",
								    d->bus, d->dev, d->func);
						else {
	...
	if (flags & PCI_FILL_ROM_BASE) {
		...
		if (reg) {
			u32 rba = pci_read_long (d, reg);
			if (rba & PCI_ROM_ADDRESS_ENABLE)
				d->rom_base_addr = rba;
		}
	}
