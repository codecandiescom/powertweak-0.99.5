  ...
  TFilterValidator("xX0123456789+-")
{
  Min = aMin;
  Max = aMax;
  if (Min >= 0) strncpy(ValidChars,"xX0123456789+",14);
  if (Min < 0 && Max < 0) strncpy(ValidChars,"xX0123456789-",14);
}
  ...
  TVTransfer Flag)
{
  ...
  switch (Flag)
  {
    ...
    case vtSetData:
      if (Min >= 0)
        snprintf(S,8,"%lu",*((unsigned long *)Buffer));
      else
        snprintf(S,8,"%ld",*((long *)Buffer));
      break;
    default:
      break;
  }
