...
void TFileInfoPane::draw()
{
    ...
    strncpy( path, ((TFileDialog *)owner)->directory,PATH_MAX-1 );
    strncat( path, ((TFileDialog *)owner)->wildCard,PATH_MAX-1-strlen(path) );
    fexpand( path );
    ...
    if ( *(file_block.name) != EOS )
    {
      ...
      if ( time->tm_hour == 0 )
        time->tm_hour = 12;
      snprintf(buf,9,"%02d",time->tm_hour);
      b.moveStr( 38, buf, color );
      b.putChar( 40, ':' );
      snprintf(buf,9,"%02d",time->tm_min);
